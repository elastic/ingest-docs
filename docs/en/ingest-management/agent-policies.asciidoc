...
When you create an integration policy you often need to provide sensitive information such as an API key or a password. To help ensure that data can't be accessed inappropriately, any secret values used in an integration policy are stored separately from other all policy details.

As well, after you've saved a secret value in {fleet}, the value can be updated but in can't be viewed either in the UI or in the policy definition. When you view the policy definition (**Actions -> View policy**), any secret values are hidden and an environment variable is displayed instead, for example `${SECRET_0}`.

To update a secret value used in an integration policy:
