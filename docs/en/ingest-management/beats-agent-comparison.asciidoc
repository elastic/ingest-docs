...
|{y}
|{y}
|{fleet}-managed {agent}s, Standalone {agent}s and {beats} require root permission only if it's configured to capture data that requires that level of permission.
