...
Refer to steps to enable or disable the following data stream features:
...
To disable TSDS, follow the preview procedure, but specify `null` for `index.mode` instead of `time_series`.
