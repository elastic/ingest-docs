...
|{agent} needs to be able to read the `/var/log/syslog` and `/var/log/auth.log` files. Add the `elastic-agent-user` to the `adm` group on the host to fix the missing visualizations.
