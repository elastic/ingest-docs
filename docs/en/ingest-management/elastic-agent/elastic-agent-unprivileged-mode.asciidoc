...
Note that this change requires {agent} to be re-enrolled, which triggers a loss of state and any associated issues, including the possibility of duplicated ingested data. Also, changing to `unprivileged` mode is prevented if the agent is currently enrolled in a policy that includes an integration that requires administrative access, such as the {elastic-defend} integration.
