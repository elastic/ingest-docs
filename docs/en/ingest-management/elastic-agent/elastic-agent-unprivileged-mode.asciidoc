...
= Run {agent} without root privileges
...
In general, agents installed without full administrative privileges will perform and behave exactly as those installed by a superuser. There are certain integrations and datastreams that are not available, however. If an integration requires root access, this is <<unprivileged-integrations,indicated on the integration main page>>.
...
The following tables show, for different operating systems, the impact on {agent} commands when the agent is <<unprivileged-running,run in unpriviledged mode>>.
...
If an integration requires {agent} to have root privileges, this is indicated at the top of the page for that integration in {kib}:

[role="screenshot"]
image::images/integration-root-requirement.png[Elastic Defend integration page showing root requirement]

A warning is displayed in {kib} if you try to add an integration that requires root privileges to an {agent} policy that has agents enrolled in unprivileged mode
