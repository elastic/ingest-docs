...
<<<<<<< HEAD
Minimum requirements have been determined by running the {agent} (`v8.9.0`) on a GCP `e2-micro` instance (2vCPU/1GB).
=======
Minimum requirements have been determined by running the {agent} on a GCP `e2-micro` instance (2vCPU/1GB).
>>>>>>> b94e040 (Remove tested version number from agent minimum install reqs (#360))
