...
Out testing was performed on AWS m7i.large instance type that has 2 vCPUs, 8.0 GiB of Ram and up to 12.5 Gibps of bandwidth. We ingested a 1.1GB single apache JSON log file, running with the throughput preset and self-monitoring enabled.
|===
| **CPU** | ~80% total, including all monitoring processes
| **RSS memory size** | ~310 MB, including all processes
| **Write network throughput** | ~3.6 MB/s
