...
If you need to limit the amount of CPU consumption you can use the `agent.limits.go_max_procs` configuration option. This parameter sets how many CPUs the Go runtime can schedule Go routines on for both the agent and the underlying {beats} that it supervises. The `agent.limits.go_max_procs` option accepts any integer value.
