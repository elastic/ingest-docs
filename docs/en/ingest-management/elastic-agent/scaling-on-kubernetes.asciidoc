...
Although daemonset installation is simpler, it can not diffrentiate the alternative agent resource requirements depending on the collected metrics. The need for better 
resource assighnment led us to alternatives installation methods. 
...
> Above tests run with {agent} version 8..8 and scraping period of 10sec in Kubernetes Policy. Those numbers are just indicators and should be validated per different kubernetes policy configuration, along with applications that the kubernetes cluster might include
...
== Visualisation Configuration best practices
