...
- Node level {agent}(no leader Agent) in a Daemonset 

- kube-state-metrics shards and {agent} in the StatefulSet defined in the kube-state-metrics autosharding manifest
 
Each of these groups of {agent} will have its own policy specific to its function and can be resourced independently in the appropriate manifest to accomodate its specific resource requirements.
...
Based on our https://github.com/elastic/elastic-agent/blob/ksmsharding/docs/elastic-agent-scaling-tests.md[{agent} scaling tests], the following table provides number of KSM sharding configuration per different {k8s} sizes:

The following table aims to assist users on how to configure their KSM Sharding as {k8s} cluster scales:
|===
