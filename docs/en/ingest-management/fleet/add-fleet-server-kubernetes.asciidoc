...
[NOTE]
====
For ECK users, if your {stack} is orchestrated by ECK, we recommend to orchestrate the {fleet-server} through the operator. That simplifies the process as all needed resources will be configured by ECK.
+
Refer to {eck-ref}/k8s-elastic-agent.html[Run {agent} on ECK] for more information.
====

[IMPORTANT]
====
This guide assumes certain knowledge about deploying applications on Kubernetes.
====
...
* A *Service token*, that you can generate following the <<add-fleet-server-kubernetes-preparations>> section, or manually using the {ref}/service-tokens-command.html[elasticsearch-service-tokens] command.
...
* Extra TLS/SSL certificates and configuration parameters in case of requiring <<mutual-tls,mutual TLS>> (not covered on this document).
...
In this flow the {fleet-server} acts as the server and the {agent} acts as the client, therefore the {fleet-server} requires a TLS certificate and key, and the {agent} needs to trust the CA certificate used to sign the {fleet-server} certificate.

[NOTE]
====
The {fleet-server} certificate is not required when installing the server using the *Quick start* mode, but should always be used for *production* deployments. In *Quick start* mode, the {fleet-server} uses a self-signed certificate and the {agent}s have to be enrolled with the `--insecure` option.
====
...
. In {kib}, open *Fleet → Settings* and ensure the *Elasticsearch output* that will be used by the {fleet-server} policy is correctly configured, paying special attention to:
+
** The *hosts* field includes a valid URL that will be reachable by the {fleet-server} Pod.
** If your {es} cluster uses certificates signed by private or intermediate CAs not publicly trusted, ensure to add the trust information in the *Elasticsearch CA trusted fingerprint* field or in the *advanced configuration* section through `ssl.certificate_authorities` setting. For more details refer to https://elastic.co/guide/en/fleet/current/secure-connections.html#_encrypt_traffic_between_elastic_agents_fleet_server_and_elasticsearch[Secure Connections] documentation.
+
[IMPORTANT]
====
...
. From *Fleet → Agent Policies*, select *Create agent policy* to create a policy for the {fleet-server}:
+
...
+
Do not continue with the next steps if the {fleet-server} is not completely up and healthy and appears in Fleet UI.

. Expose the {fleet-server} to your clients through the *Fleet Server Host*:
+
This may include the creation of a Kubernetes `service`, an `ingress` resource, or create DNS registers for FQDNs resolution. There are multiple ways to expose the Pods in Kubernetes.
+
...
[source, shell]
------------------------------------------------------------
kubectl expose deployment fleet-server --type LoadBalancer --port 443 --target-port 8220
------------------------------------------------------------
+
The previous example creates a service named `fleet-server`, listening on port `443` and forwarding the traffic to the `fleet-server` deployment's Pods on port `8220`. The listening `--port` of the service can be customized, but the `--target-port` must remain on the default port (`8220`), because it's the port used by the {fleet-server} application.
+
...
* For external clients: access through whatever FQDN added on the DNS server pointing to the public IP of the `LoadBalancer`.
+
If your environment or network architecture requires the {fleet-server} to be reachable through multiple hostnames / URLs, you can create multiple *Fleet Server Hosts* in *Fleet -> Setting*, and create different policies for different groups of agents.
+
...
* TLS certificate trust issues even when `ELASTICSEARCH_CA` environment variable is properly set during deployment:
+
This occurs when the `output` of the policy associated to the {fleet-server} is not correctly configured. Add the *CA certificate* or *CA trusted fingerprint* to the {es} output associated to the {fleet-server} policy.
...
To learn how, see <<install-fleet-managed-elastic-agent>>, or <<running-on-kubernetes-managed-by-fleet>> if your {agent}s will also run on Kubernetes.

When connecting {agent}s to this {fleet-server}, remember to use `--insecure` flag if the *quick start* mode was used, or to provide to the {agent}s the CA certificate associated to the {fleet-server} certificate if *production* mode was used.