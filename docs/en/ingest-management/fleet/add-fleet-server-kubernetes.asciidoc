...
** Optionally, you can add an **inactive agent unenrollment timeout** to automatically unenroll and invalidate API keys after they become inactive. This is useful on Kubernetes environments as the {fleet-server} Pods are ephemeral, and after restarts new {agent}s will appear in Fleet UI.
