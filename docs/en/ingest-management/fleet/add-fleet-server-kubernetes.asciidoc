...
* Plan the SSL setup, server certificates, {fleet-server} URL, and certificate authorities (CAs).
...
+
You also need to know and plan in advance the `URL` (or `URLS`) that the {agent} clients will use to access the {fleet-server}, because the *hostname* part of the URL needs to be included in the server certificate as an `x.509 Subject Alternative Name (SAN)`. If you plan to make your {fleet-server} accessible in multiple *hostnames*, add all of them to the server certificate.
+
...
** This will be the URL used by the clients ({agent}s) to connect to the {fleet-server}. In *Production* mode, the {fleet-server} certificate must include the hostname part of the URL in its x509 SAN extension.
** On Kubernetes environments this should be the name of the service configured for the {fleet-server}.
** You can add multiple URLs here: This is useful on Kubernetes if the {fleet-server} is reachable internally through one URL and externally through a different URL.
