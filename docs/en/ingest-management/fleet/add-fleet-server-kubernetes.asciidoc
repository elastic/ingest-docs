...
Before deploying the {fleet-server}, you need to:
...
In this flow the {fleet-server} acts as the server and the {agent} acts as the client, therefore the {fleet-server} requires a TLS certificate and key, and the {agent} will need to trust the CA certificate used to sign the {fleet-server} certificate.
...
In this flow the {fleet-server} acts as the client and {es} acts as the HTTPS server. For the communication to succeed, the {fleet-server} needs to trust the CA certificate used to sign the {es} certificate. If your {es} uses certificates signed by a corporate CA or multiple intermediate CAs you will need to use them during the {fleet-server} setup.
...
+
* For {agent}s within the Kubernetes cluster: `https://fleet-server.<namespace>.svc`.
* For external {agent}s: access through whatever FQDN added on the DNS server pointing to the public IP of the `LoadBalancer`.
+
If your environment requires the {fleet-server} to be reachable through multiple hostnames / URLs, you can create multiple *Fleet Server Hosts* in *Fleet → Setting*, and create different policies for different groups of agents.
+
