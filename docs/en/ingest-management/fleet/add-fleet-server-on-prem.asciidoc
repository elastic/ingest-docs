...
. When you **Add your {fleet-server} host**, if the **Edit hosts** fields are 
grayed out, {fleet-server} hosts are configured outside of {fleet}. 
For more information, refer to {kibana-ref}/fleet-settings-kb.html[{fleet} 
settings in {kib}].
====
...
+
You can update your {fleet-server} configuration in {kib} at any time by going 
to *Management* -> *{fleet}* -> *Settings*. From there, you can:
