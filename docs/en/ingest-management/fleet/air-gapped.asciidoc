...
The {package-registry} must be therefore accessible from {kib} via an HTTP Proxy and/or self-hosted.

The {artifact-registry} must be therefore accessible from {kib} via an HTTP Proxy and/or self-hosted.
