...
{serverless-full}::
In a link:{serverless-docs}[{serverless-short}] environment, {fleet-server} is configured and managed automatically, beneath the covers. There's nothing that you need to configure.
