...
You can drag and drop processors that you've defined in order to change the order in which they're applied to events.

For example, you might route log events based on severity. To do so, you can specify a default topic for all events not matched by other processors:
...
| Select a compression codec to use. Supported codecs are `snappy`, `lz4` and `gzip`. The default is `gzip`.
...
| The maximum length of time a Kafka broker waits for the required number of ACKs (see the `ACK reliability` setting) before timing out. The default is 10 seconds.
