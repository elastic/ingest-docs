...
|
[id="kafka-output-authentication"]
**Authentication**
...
Provide a username and password for connecting to Kafka.

Select a Simple Authentication and Security Layer (SASL) mechanism for your login credentials:
...
Use the Secure Sockets Layer (SSL) protocol to authenticate. 

* **Server SSL certificate authorities**: The CA certificate to use to connect to Kafka. This is the CA used to generate the certificate and key for Kafka. Copy and paste in the full contents for the CA certificate.
+
This setting is optional.
...
Use Kerberos authentication between {agent} and Kafka.

// =============================================================================

|
[id="kafka-output-partitioning"]
**Partitioning**

| Select a Kafka output broker event partitioning strategy. By default the random partitioner is used.

When you select **Random** or **Round robin**, you must also specify the number of events to be published to the same partition, before the partitioner selects a new partition, either at random or by a round robin sequence.

When you select **Hash**, you must also specify the list of fields used to compute the partitioning hash value from. If no field is configured, the events `key` value will be used.

// =============================================================================

|
[id="kafka-output-topics"]
**Topics**

| Use these settings to dynamically set the topic for an event
