...
. Click **Create rule**.
. Select **Elasticsearch query** as a rule type.
...
** SIZE: 100
. Set **Check every** to the interval at which you'd like to run the query, for example, 1 minute.
. Select an action to occur when the rule conditions are met. For example, you can set the alert to send an email, specify:
** Email connector: `Elastic-Cloud-SMPT`
** Action frequency: `For each alert` `On status changes`
