...
In {fleet}, you can access the detailed status for of individual agent and the integrations that are associated with it through the agent policy.

. In {kib}, go to **Management -> {fleet} -> Agents**.

. In the **Host** column, click the agent's name.

On the **Agent details** tab, the **Overview** pane shows details about the agent and its performance, including its memory and CPU usage, last activity time, and last checkin message. You can also <<view-agent-metrics>> in {kib}.
