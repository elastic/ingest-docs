...
Under routine circumstances, an {agent} upgrade happens quickly. You typically will not see the agent transition through each of the upgrade states. However, the detailed upgrade status can be a very useful tool to diagnose the state of an agent that may be stuck, or appears to be stuck, during the upgrade process.
