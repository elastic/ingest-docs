...
On the **Agents** tab, when you trigger an upgrade, agents that are currently 
upgrading show a detailed upgrade status indicator beside the agent version.

[role="screenshot"]
image::images/upgrade-states.png[Detailed state of an upgrading agent]

The following table explains the various possible upgrade states.
...
| Upgrade rolled Back | Due to an error detected in the newly upgraded {agent}, it has been rolled back to the previous version.
| Upgrade scheduled | {agent} has received the upgrade notice from {fleet} and the upgrade will start at the indicated time.

|===

As well, for {agents} on version 8.11 and higher, you can hover your cursor over the information icon to get more granular details about the upgrade state.
