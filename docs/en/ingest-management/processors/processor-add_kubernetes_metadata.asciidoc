...
Wildcards are supported in these settings by using `use_regex_include: true` in case of `include_labels` and respectively set `use_regex_exclude: true` in combination with `exclude_labels`.
