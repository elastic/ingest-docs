...
====
*Details*
An issue discovered in {fleet-server} that causes Fleet Server to fail to bootstrap with self-signed certificates. This is due to an update in beats dependencies where the certificate validation was completely rewritten.
