...
IMPORTANT: In an air-gapped environment, the agent won't be able to download the remote key and therefore cannot be upgraded. If you're using an air-gapped environment, we recommended waiting for this issue to be resolved before upgrading to 8.9.x or a higher version, to avoid being unable to upgrade.
