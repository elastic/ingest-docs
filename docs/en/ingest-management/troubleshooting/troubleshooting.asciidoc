...
Another possible solution to fix this without force unenroll is to use the 'dev tools' to force the stale {agent} to upgrade:

[source,shell]
----
POST kbn:/api/fleet/agents/5f18484d-17d5-12fe-c455-fcde60f8b/upgrade
{ "force": true, "version": "8.7.0" }
----

In this example, we are upgrading the agent with agent-id *5f18484d-17d5-12fe-c455-fcde60f8b* to version *8.7.0*. The dev tools can be found by browsing to Management -> Dev Tools of your instance. The agent-id of an Agent can be found by browsing to Management -> Fleet -> Search for the Agent based, for example, hostname.
